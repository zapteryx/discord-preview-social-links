  ...
  try {
    ...
    for (let i = startIndex; i < availableFixers.length; i++) {
      ...
      if (!isFixerOnCooldown(matchedDomain, fixer)) {
        // Preserve subdomain when replacing the domain
        let newHostname;
        if (hostname === matchedDomain) {
          // No subdomain
          newHostname = fixer;
        } else if (hostname.endsWith('.' + matchedDomain)) {
          // Has subdomain - extract and preserve it
          const subdomain = hostname.slice(0, -(matchedDomain.length + 1));
          newHostname = `${subdomain}.${fixer}`;
        } else {
          // Shouldn't happen, but fallback to simple replace
          newHostname = fixer;
        }

        const newUrl = originalUrl.replace(hostname, newHostname);
